...
class ForecastFloodRiskProcess(Process):
    def __init__(self):
        inputs = [ComplexInput('fcst', 'Streamflow forecasts (ensemble or deterministic)',
                  ...
                  LiteralInput('name', 'Name of the variable within the fcst netcdf file.',
                               abstract="Name of the variable within the fcst netcdf file to access the data using a variable naming scheme",
                               data_type='string',
                               min_occurs=1,
                               max_occurs=1),
                  LiteralInput('flood_level', 'Flood level threshold.',
        ...
        outputs = [ComplexOutput('flood_risk', 'Empirical flood risk based on forecast data and specified flood level.',
        ...
        super(ForecastFloodRiskProcess, self).__init__(
            ...
            inputs=inputs,
            outputs=outputs,
            keywords=["forecast","flood","risk","threshold","ensemble","deterministic"],
    ...
    def _handler(self, request, response):
        ...
        if 'member' in fcst.coords:
            ...
            pct=fcst.where(fcst>flood_level).notnull().sum(dim='member',keep_attrs=True)/number_members
            
        #it's deterministic:
        else:
            pct=fcst.where(fcst>flood_level).notnull()/1.0 # This is needed to return values instead of floats
