...
from pssh.exceptions import AuthenticationException, UnknownHostException, \
                            ConnectionErrorException, SessionError, SFTPIOError
...
class HPCConnection(object):
    ...
    #    df_basename = data_path_content[0]
        ...
        print("system cmd: "+"tar cvf /tmp/" + remote_temp_folder +
...
def process_cmd(executable_, client, hostname, option, jobname="", jobid=""):
    ...
    if option == "Submit":
        ...
        copy_data_to_remote(client, src_data_path, dataset, remote_path, remote_temp_folder)

        script_fname = copy_batchscript(executable_, client, remote_path, remote_temp_folder,
                                        dataset, batch_cmd_template)
