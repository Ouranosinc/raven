    ...
    if RunSuccessLandUse[i-1]==0:
        for attempt in range(1):
            try:
                ...
                # Save file
                zippedList =  list(zip(ID, StationID, Area, Latitude,Longitude,Gravelius,	Perimeter,	RunSuccessShape,Elevation,	Slope,Aspect,RunSuccessTerrain,Forest,Grass,	Wetland,Water,Urban,Shrubs,Crops,	SnowIce,RunSuccessLandUse)) 
                frameList = pd.DataFrame(data=zippedList, columns=['ID', 'StationID', 'area', 'latitude','longitude','gravelius','perimeter','RunSuccessShape','elevation','slope','aspect','RunSuccessTerrain','forest','grass','wetland','water','urban','shrubs','crops','snowIce','RunSuccessLandUse'])
                frameList.to_csv('gauged_catchment_properties.csv')
            except:
                print('Failed, restarting iter ' + str(attempt))  
            else:
                break
