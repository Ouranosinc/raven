...
class TestAssimilationGR4JCN:
    def test_simple(self):
        ...
        xa = [
            model.storage["Soil Water[0]"].isel(time=-1).values,
            model.storage["Soil Water[1]"].isel(time=-1).values,
        ]
        ...
        [xa, q_assim, q_obs, model, solutions_end] = assimilateQobsSingleDay(
            model, xa, ts, date_list, std, solutions=solutions_end
        )
        ...
        number_iterations = (
            4  # let's just do 4 iterations for this test case to keep things short
        )
        ...
        for i in range(number_iterations):
            ...
            [xa, q_a, q_o, model, solutions_end] = assimilateQobsSingleDay(
                model, xa, ts, date_list, std, solutions=solutions_end
            )
        ...
        end_date = start_date + dt.timedelta(
            days=10 + number_iterations * assim_timestep - 1
        )
